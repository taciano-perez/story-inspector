...
public class AnnotationEngine {
    ...
    private static void inspectSentiments(final Chapter chapter) {
        for (final Block block : chapter.getBlocks()) {
            ...
            final double unweightedScore = SentimentInspector.inspectSentimentScore(block.getBody());
            final double weight = Double.parseDouble(block.getWordCount()) / NR_WORDS_PER_BLOCK;
            final double weightedScore = unweightedScore * weight;
            block.setSentimentScore(Double.toString(weightedScore));
        }
    }
